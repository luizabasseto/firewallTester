...
class FirewallGUI:
    ...
    def create_firewall_tab(self):
        ...
        #     header_width = tk_font.measure(field) + 20
        #     self.tree.column(field, width=header_width, stretch=False)
        ...
        self.button_tree_add = tk.Button(self.tests_frame_Tree, text="Adicionar", command=self.add_entry)
        self.button_tree_add.pack(side="left", padx=5, pady=11)
        
        self.button_tree_edit = tk.Button(self.tests_frame_Tree, text="Editar", command=self.edit_entry)
        self.button_tree_edit.pack(side="left", padx=5, pady=11)

        self.button_tree_del = tk.Button(self.tests_frame_Tree, text="Deletar", command=self.delete_entry)
        self.button_tree_del.pack(side="left", padx=5, pady=11)
        
        self.button_tree_test = tk.Button(self.tests_frame_Tree, text="Testar linha", command=self.testar_linha_tree)
        self.button_tree_test.pack(side="left", padx=5, pady=11)
        
        self.button_tree_test_all = tk.Button(self.tests_frame_Tree, text="Testar tudo", command=self.executar_todos_testes_thead_tree)
        self.button_tree_test_all.pack(side="left", padx=5, pady=11)
    ...
    def on_tree_select(self, event):
        ...
        if selected_item:
            ...
            if item_values:
                #container_id = self.hidden_data.get(item_values[0], "")  # Recupera Container ID
                #full_values = [item_values[0], container_id] + list(item_values[1:])
                print(f"{item_values}")
    ...
    def is_duplicate(values):
        ...
        for item in self.tree.get_children():
            existing_values = self.tree.item(item, "values")  # Obtem os valores existentes
    ...
    def add_entry(self):
        ...
        #self.validar_e_adicionar_teste()
        ...
        #item_id = self.tree.insert("", "end", values=[values[0]] + values[2:])  # Armazena Teste ID e oculta Container ID
        #item_id = self.tree.insert("", "end", values=[values[0]] + values[2:])  # Armazena Teste ID e oculta Container ID
        values=[]
    ...
    def edit_entry(self):
        ...
        #values = [self.tree.item(selected_item, "values")[0]] + [entry.get().strip() for entry in entries]
        src_ip = self.src_ip.get()
        dst_ip = self.dst_ip.get()
        protocol = self.protocol.get()
        src_port = self.src_port.get()
        ...
        #apply_row_color(selected_item, values[-1])  
        #clear_entries()
